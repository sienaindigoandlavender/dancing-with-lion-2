...
export default function TheHealthLeapfrog() {
  return (
    <div className="bg-white text-[#262626] pt-16">
      ...
      <section className="border-t border-[#e5e5e5] bg-[#fafafa]"><div className="max-w-[800px] mx-auto px-6 md:px-10 py-20 min-h-[40vh] flex items-center">
        <blockquote className="border-l-[3px] pl-6 md:pl-8" style={{ borderColor: '#991B1B' }}><p className="font-serif text-[24px] md:text-[32px] italic leading-[1.4] text-[#0a0a0a]">Africa has 3% of the world&rsquo;s health workers and 25% of the disease burden. The leapfrog is not optional — it is survival.</p><p className="text-[11px] text-[#a3a3a3] mt-4 uppercase tracking-wider">Dancing with Lions analysis</p></blockquote>
      </div></section>
      ...
      <section className="border-t border-[#e5e5e5] bg-[#0a0a0a]"><div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16 md:py-24">
        ...
        <div className="space-y-6">{[{ to: 'The Tech Leapfrog', link: '/data/the-tech-leapfrog', note: 'M-Pesa → M-TIBA. The mobile money platform becomes the health insurance platform.' }, { to: 'The Demographic Dividend', link: '/data/the-demographic-dividend', note: 'The dividend requires a healthy workforce. Malaria costs Africa $12B/year in lost productivity.' }].map((c, i) => <div key={i} className="border-l-[3px] pl-6" style={{ borderColor: '#991B1B' }}><Link href={c.link} className="text-[10px] text-[#f87171] uppercase tracking-[0.1em] underline hover:no-underline font-semibold">{c.to}</Link><p className="text-[13px] text-white/50 mt-1.5">{c.note}</p></div>)}</div>
      </div></section>
      ...
      <section className="border-t border-[#e5e5e5] bg-[#fafafa]"><div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16">
        ...
        <div className="pt-6 flex justify-between items-center border-t border-[#e5e5e5]"><p className="text-[12px] text-[#737373]">Research &amp; analysis: <strong className="text-[#0a0a0a]">Dancing with Lions</strong></p><p className="text-[11px] text-[#737373]">&copy; Dancing with Lions 2026</p></div>
      </div></section>
    </div>
  )
}
