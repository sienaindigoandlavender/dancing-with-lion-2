...
export default function TheConservationDeficit() {
    ...
    <div className="bg-white text-[#262626] pt-16">
      ...
      <section className="border-t border-[#e5e5e5] bg-[#0a0a0a]"><div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16 md:py-24">
        ...
        <div className="space-y-10">
          {[
            { to: 'The Gorilla Dividend', link: '/data/the-gorilla-dividend', insight: 'The success model. $1,500 permits, 10% to communities, former poachers as trackers. 254 → 1,063. The only great ape increasing.' },
            { to: 'The Lion Economics', link: '/data/the-lion-economics', insight: 'The failure model. 200,000 → 23,000. $20.5B safari market, $619M funding gap. The species funding its own extinction.' },
            { to: 'The Last Lions', link: '/data/the-last-lions', insight: 'The Barbary lion went extinct because nobody built an economic model around keeping it alive. The Atlas lion is the historical warning.' },
            { to: 'The Sahel War', link: '/data/the-sahel-war', insight: 'Conflict zones are extinction zones. 60% of Burkina Faso outside state control. W-Arly-Pendjari lost 7,000 elephants. Wagner replaced France. Wildlife got worse.' },
            { to: 'The Blood Gold', link: '/data/the-blood-gold', insight: 'Wagner/Africa Corps extract $2.5B+ in gold from 7 African countries. Zero tax paid. The same networks that mine gold facilitate ivory and pangolin trafficking.' },
            { to: 'The Silk Road Into Africa', link: '/data/the-silk-road-into-africa', insight: 'BRI railways and roads cut through wildlife corridors. $182.3B in infrastructure loans. The question: does infrastructure enable conservation tourism or habitat fragmentation?' },
          ].map((c, i) => (
            <div key={i} className="border-l-[3px] pl-6 md:pl-8" style={{ borderColor: '#991B1B' }}>
              <Link href={c.link} className="text-[10px] text-[#f87171] uppercase tracking-[0.1em] underline hover:no-underline font-semibold">{c.to}</Link>
      ...
      <section className="border-t border-[#e5e5e5] bg-[#fafafa]"><div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16">
        ...
        <div className="mt-8 pt-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-t border-[#e5e5e5]">
          <p className="text-[12px] text-[#737373]">Data compilation, visualisation &amp; analysis: <strong className="text-[#0a0a0a]">Dancing with Lions</strong></p>
