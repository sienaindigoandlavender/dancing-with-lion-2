...
const INCIDENTS = [
  { date: 'Mar 27–31, 2022', actor: 'Wagner + FAMa', location: 'Moura, Mopti (Mali)', dead: '500+', detail: '~100 Wagner fighters + Malian army. 3,000 rounded up at market. Executed by shooting in back. 58 women/girls raped. OHCHR confirmed.', type: 'massacre', link: '/data/the-blood-gold' },
  { date: 'Apr 2022', actor: 'Wagner', location: 'Gossi (Mali)', dead: '~12', detail: 'Wagner buried bodies near base, produced footage blaming French. France released satellite imagery exposing false flag.', type: 'false-flag', link: '/data/the-blood-gold' },
  ...
  { date: 'Jul 25–27, 2024', actor: 'Tuareg vs Wagner', location: 'Tinzaouaten (Mali)', dead: '20–80 Wagner', detail: 'CSP-DPA rebels ambush Wagner-Malian convoy. Wagner\'s worst military loss in Mali. Ukraine claimed assistance.', type: 'military', link: '/data/the-blood-gold' },
  ...
  { date: 'Sep–Nov 2025', actor: 'JNIM', location: 'Kayes region (Mali)', dead: 'Multiple', detail: 'Fuel blockade. Hit tanker convoys from the west. More strikes in 2025 than prior 5 years combined. National fuel stocks depleted. Mass evacuations.', type: 'siege', link: '/data/the-atlantic-spine' },
...
export default function TheSahelWar() {
                ...
                {INCIDENTS.map((inc, i) => (
                  <tr key={i} className="border-b border-[#e5e5e5] align-top">
                    ...
                    <td className="py-2.5 text-[10px]">
                      {inc.link ? (
                        <Link href={inc.link} className="text-[#991B1B] underline hover:no-underline">
      ...
      <section className="border-t border-[#e5e5e5] bg-[#0a0a0a]">
        <div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16 md:py-24">
          ...
          <div className="space-y-10">
            {[
              { to: 'The Blood Gold', link: '/data/the-blood-gold', insight: 'Wagner/Africa Corps operates in the same countries where JNIM and ISSP are expanding. In Mali, junta + Russian forces killed more civilians (76% of fatalities) than jihadists in 2024. ACLED: security force violence against civilians increased 76% between 2022–2024. Violence drives recruitment. The counterterrorism operation became the recruitment operation.' },
              { to: 'The Atlantic Spine', link: '/data/the-atlantic-spine', insight: 'The Nigeria-Morocco Gas Pipeline passes through JNIM territory. The Trans-Saharan Pipeline crosses Niger — an AES junta state. JNIM\'s fuel blockade of Kayes (Sep 2025) demonstrated that a jihadi group can cut an entire country\'s energy supply. Pipeline infrastructure in the Sahel is a target, not just a corridor.' },
              { to: 'The Lake of Fire', link: '/data/the-lake-of-fire', insight: 'ISSP\'s Lakurawa subgroup is expanding from Niger into Nigeria\'s Sokoto and Kebbi states — meeting ISWAP and Boko Haram from the other direction. The Sahel war and the Lake Chad war are merging into a single interconnected conflict zone.' },
            ].map((c, i) => (
