...
export default function TheEducationGap() {
  return (
    <div className="bg-white text-[#262626] pt-16">
      ...
      <section className="border-t border-[#e5e5e5] bg-[#0a0a0a]"><div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16 md:py-24">
        ...
        <div className="space-y-6">{[{ to: 'The Demographic Dividend', link: '/data/the-demographic-dividend', note: 'The dividend is only a dividend if the population is educated. Without education, demographics become a liability.' }, { to: 'The Tech Leapfrog', link: '/data/the-tech-leapfrog', note: 'Coding academies, EdTech, mobile learning — the tech leapfrog is the education leapfrog.' }, { to: 'The Creative Economy', link: '/data/the-creative-economy', note: 'Nollywood, Afrobeats, fashion — creative industries need educated creators who understand IP, contracts, distribution.' }].map((c, i) => <div key={i} className="border-l-[3px] pl-6" style={{ borderColor: '#0369A1' }}><Link href={c.link} className="text-[10px] text-[#60a5fa] uppercase tracking-[0.1em] underline hover:no-underline font-semibold">{c.to}</Link><p className="text-[13px] text-white/50 mt-1.5">{c.note}</p></div>)}</div>
      </div></section>
